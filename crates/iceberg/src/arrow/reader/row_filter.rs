...
impl ArrowReader {
    ...
    /// unchanged.
    pub(super) async fn prune_row_groups_with_bloom_filters<R>(
        predicate: &BoundPredicate,
...
message schema {
  ...
  required group nested = 2 {
    ...
    /// cannot disprove `id = 50`, but the bloom filter knows only `1` and `100`
    /// were inserted.
    #[tokio::test]
    async fn test_bloom_filter_prunes_row_group_when_value_definitely_absent() {
        ...
        let id_data = Arc::new(Int32Array::from(vec![1_i32, 100])) as ArrayRef;
    ...
    /// A predicate value that is present in the bloom filter must not be pruned
    /// and should be returned through the regular row-filter path.
    #[tokio::test]
